...
- A brief aside:
    + Let's say of all the functions/constants/etc. in **math**, you only care about `math.pi`
...
# Same thing, but no `math` at the start
print(f'pi = {pi:.3f}')
```
...
- Let's install the packages **NumPy** and **pandas**!

<!-- GK: Share screen here and show how to add numpy and pandas to environment -->

## Importing Packages

As an example (and a test to see if **NumPy** is installed), let's explore the `np.random.normal()` function, which generates random values from a normal distribution: \pause
...
numpyArray = np.array(list1)  # Convert list -> numpy array
print(numpyArray * 2)
...
- The distinction between *methods* and *attributes* is that an attribute gives access to information associated with a variable; whereas a method is a function that may act on/with the variable.
    ...
    + E.g., `.shape` (attribute) with **NumPy** arrays and `.split()` (method) for strings
:::
...
x = np.random.normal(loc = 0, scale = 10, size = (3,3))
...
### Challenge:
- Group `df` by `Age` and then get the mean of the rating column (for each `Age` group) \pause

```{python}
#| label: groupbyChallenge
