...
plt.annotate(
    ...
    color = "white", 
    fontsize = 25, fontfamily='signpainter'
)

plt.annotate(
    "Spring 2026", (0, -.83), ha = "center", va = "center",
    color = "white", 
    fontsize = 10, fontfamily='signpainter'
)
...
axin = ax.inset_axes([.3285, .405, 0.15, 0.15]) # This creates the inset graph; args: [x, y, width, height]
axin.plot(point_x, point_y, 'wo', markersize = 2.5) # Plot points
axin.plot(line_x, line_y, '-', color = "turquoise", linewidth = 2) # Plot line
...
plt.savefig('images/logo.png', dpi=300, transparent=True)
plt.show()